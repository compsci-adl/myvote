...
const config: Config = {
  ...
  // cacheDirectory: "/private/var/folders/0l/95fwcmkd7zv8czj9jzz3mhp00000gn/T/jest_dx",
  ...
  // Use ts-jest ESM preset so TypeScript + JSX tests run without additional build step
  preset: 'ts-jest/presets/default-esm',

  // Ensure ts/tsx files are transformed by ts-jest with ESM support and use a Jest-focused tsconfig
  transform: {
    '^.+\\.(ts|tsx)$': ['ts-jest', { useESM: true, tsconfig: '<rootDir>/tsconfig.jest.json' }],
  },

  // Treat TypeScript files as ESM modules so JSX in .tsx tests is parsed/transformed
  extensionsToTreatAsEsm: ['.ts', '.tsx'],

  // The test environment that will be used for testing (browser-like)
